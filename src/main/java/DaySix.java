...
public class DaySix {
    ...
    private void setup() {
        for(int i = 0; i < 1000; i++) {
            Arrays.fill(lightGrid[i], 0);
        }
    }

    private void performActions(String input) {
    ...
    private void toggleLights(String action) {

        String[] values =  action.split(" ");
        int[] startPoints = Arrays.stream(values[1].split(",")).mapToInt(Integer::parseInt).toArray();
        int[] endPoints = Arrays.stream(values[3].split(",")).mapToInt(Integer::parseInt).toArray();

        for(int x = startPoints[0]; x <= endPoints[0]; x++) {
            for(int y = startPoints[1]; y <= endPoints[1]; y++) {
                lightGrid[x][y] = lightGrid[x][y] == 1 ? 0 : 1;
            }
        }
    }

    private void turnOffLights(String action) {
        String[] values =  action.split(" ");
        int[] startPoints = Arrays.stream(values[2].split(",")).mapToInt(Integer::parseInt).toArray();
        int[] endPoints = Arrays.stream(values[4].split(",")).mapToInt(Integer::parseInt).toArray();
    ...
    private void turnOnLights(String action) {
        String[] values =  action.split(" ");
        int[] startPoints = Arrays.stream(values[2].split(",")).mapToInt(Integer::parseInt).toArray();
        int[] endPoints = Arrays.stream(values[4].split(",")).mapToInt(Integer::parseInt).toArray();
    ...
    private void performActionsPartTwo(String input) {
        for(String action : input.split("\n")) {
            if(action.startsWith("turn on")) {
                turnOnLightsPartTwo(action);
            } else if(action.startsWith("turn off")) {
                turnOffLightsPartTwo(action);
            } else if(action.startsWith("toggle")) {
                toggleLightsPartTwo(action);
            }
        }
    }

    private void toggleLightsPartTwo(String action) {
    ...
    private void turnOffLightsPartTwo(String action) {
        String[] values =  action.split(" ");
        int[] startPoints = Arrays.stream(values[2].split(",")).mapToInt(Integer::parseInt).toArray();
        int[] endPoints = Arrays.stream(values[4].split(",")).mapToInt(Integer::parseInt).toArray();
    ...
    private void turnOnLightsPartTwo(String action) {
        String[] values =  action.split(" ");
        int[] startPoints = Arrays.stream(values[2].split(",")).mapToInt(Integer::parseInt).toArray();
        int[] endPoints = Arrays.stream(values[4].split(",")).mapToInt(Integer::parseInt).toArray();
    ...
    public static void main(String... args) {
        ...
        daySix.setup();
        daySix.performActions(input);
        System.out.printf("Part One: %d", daySix.countLitLights());
        daySix = new DaySix();
        daySix.setup();
        daySix.performActionsPartTwo(input);
        System.out.printf("%nPart Two: %d", daySix.countLitLights());
    }
